...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $program_id          = new TDBSeekButton('program_id', 'sample', 'form_System_user', 'SystemProgram', 'name', 'program_id', 'program_name');
        $program_name        = new TEntry('program_name');
        $groups              = new TDBCheckGroup('groups', 'sample', 'SystemGroup', 'id', 'name');
        ...
        $frame_groups = new TFrame(NULL, 300);
        ...
        $this->form->addFields( [new TLabel(_t('Password'))], [$senha],  [new TLabel(_t('Password confirmation'))], [$repassword] );
    ...
    public static function onSave($param)
    {
        try {
            ...
            if ( $object->senha )
            {
                ...
                {
                    (new TStrongPasswordValidator)->validate(_t('Password'), $object->senha);
                }
                
                if( $object->senha !== $param['repassword'] )
                {
